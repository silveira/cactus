...
public class SMR extends Instruction {
    ...
    public void execute(CentralProcessorUnit cpu){
        ...
        //Get the content from MBR and subtract from register AC content and put the result into register AC
        cpu.getRegister(ac).setContent(
                    Integer.toBinaryString(
                        Integer.parseInt(cpu.getRegister(ac).getContent(),2) - Integer.parseInt(cpu.getMbr().getContent(),2)
                    )
                );
    }
